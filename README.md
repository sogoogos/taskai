...
- **日別タイムライン**：右パネルの「タイムライン」タブで、選んだ日の予定を時間軸表示（現在時刻の線つき・アカウント別に色分け＋凡例）
- **タスク（ToDo）管理**：右パネルの「タスク」タブ、またはチャット（「〇〇をやることに追加」「今日のタスク見せて」「△△を完了にして」）で作成・一覧・進捗変更（未着手/着手中/完了）。期日つき・「今日」絞り込み対応
- **投資（スイング取引）状況**：右パネルの「投資」タブ、またはチャット（「今の持ち株は?」「投資の成績どう?」）で確認。外部の自動売買システム [kabu-trader](../kabu-trader)（EC2 上）が市場ごと（日本株ペーパー/日本株ライブ/米国株）の評価額・損益率・保有銘柄・直近売買を定期送信 → 表示。詳細は [docs/TRADING.md](docs/TRADING.md)
...
- フェーズ3: Slack 読み取り → 同上
- フェーズ4: ToDo タスク管理（✅ 実装済み）、バックグラウンド自動取り込み（cron）
